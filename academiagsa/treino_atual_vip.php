...
$aluno_id = isset($_GET['id']) ? intval($_GET['id']) : $_SESSION['user_id'];
...
<head>
    ...
    <style>
        ...
        .btn-voltar { color: #8899a6; text-decoration: none; font-weight: bold; background: rgba(255,255,255,0.05); padding: 8px 15px; border-radius: 10px; border: 1px solid var(--border); transition: 0.3s; }
        .btn-voltar:hover { background: var(--border); color: #fff; }
        ...
        .btn-final-save { position: fixed; bottom: 20px; left: 20px; right: 20px; background: var(--purple); color: #fff; border: none; padding: 18px; border-radius: 50px; font-weight: bold; cursor: pointer; z-index: 1000; text-align: center; }
    </style>
</head>
<body>

    <div class="header-painel">
        <h2 style="margin:0;">Ficha: <span style="color:var(--accent);"><?= htmlspecialchars($aluno['nome']) ?></span></h2>
        <a href="painel_vip.php" class="btn-voltar">← Voltar</a>
    </div>
    ...
    <?php if (empty($fichas_json)): ?>
        <div class="step-box" style="padding:40px; text-align:center; background:var(--card); border-radius:20px; border:1px solid var(--border);">
            ...
            <a href="prescrever_treinovip.php?id=<?= $aluno_id ?>" style="color:var(--accent); font-weight:bold;">Criar Novo Treino Agora</a>
        </div>
    <?php else: ?>
