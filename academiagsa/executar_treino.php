...
if (!isset($_SESSION['user_id']) || !isset($_GET['tipo'])) {
    die("Sessão expirada ou treino não selecionado.");
}
...
<head>
    ...
    <style>
        ...
        /* CARDS */
        .ex-card { background: var(--card); border-radius: 28px; margin-bottom: 20px; border: 1px solid rgba(255,255,255,0.05); overflow: hidden; position: relative; }
        ...
        .btn-finish { position: fixed; bottom: 25px; width: calc(100% - 40px); max-width: 460px; background: #162127; color: var(--accent); border: 2px solid var(--accent); padding: 20px; border-radius: 22px; font-weight: 900; z-index: 2000; font-size: 1rem; }
...
<body>
    ...
    <script>
        ...
        let start = localStorage.getItem('ov_start') || Date.now();
        if(!localStorage.getItem('ov_start')) localStorage.setItem('ov_start', start);
        ...
        function subtractSeries(id) {
            ...
            if (current > 1) { current--; pillVal.innerText = current; } 
            else { setFinished(id); }
        }
        ...
        function finishAll() {
            if(confirm("Encerrar treino e salvar no histórico?")) {
                ...
                localStorage.removeItem('ov_start');
                window.location.href = `treino_concluido.php?tempo=${durationSeconds}&tipo=<?= $tipo_treino ?>`;
            }
        }
